...
custom_region <- read_csv(paste0(input_path,
                                  "regions_R61_nuts.csv")
                                 )$region_nuts[sample(1:1160, 10)]
#NOTE: samples 100 NUTS3 regions from set [1:1165]

custom_input_scope <- "eu27"  # global or eu27

vacant_mode_selected <- "vacant"  # none or vacant
...
if (geo_level == "region_bld") {
    ...
    eu27_region <- read_csv(paste0(input_path,
                                  "regions_R61_nuts.csv")
                                 )$region_nuts
    }
...
region_settings <- switch(
  ...
  custom = list(
    ...
    label = paste0(
      "custom_",
      "NUTS"#paste(sub("^.*-","",custom_region),collapse = "-")
    ),
